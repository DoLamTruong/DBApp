...
foreign key(given_to) references Delivery(ID);

alter table Delivery_Phone
add constraint fk_phone_belong_to_delivery
foreign key(company_ID) references Delivery(ID);
...
foreign key(supplied_by) references Supplier(ID);
